...
public class Sound {
  ...
  enum eType {
    ...
    // シングルトン
    //シングルトンのため、Soundクラスのインスタンスは1つしか定義できない。新たにインスタンス化すると、コンパイルエラーになる
    //static Sound _singleton = null;
    private static Sound _singleton = new Sound();
    // インスタンス取得
    public static Sound GetInstance()
  {
    //??は左側のオペランドがnull値の場合、左側のオペランドを返し、null値の場合、右側のオペランドを返す
  ...
  public static bool PlaySe(string key, int channel=-1) {
   //GetInstance()でSoundクラス型のインスタンスを指定
   //Soundクラス型のインスタンスのメンバメソッド_PlaySeメソッドに、指定したキー,値を返す
     return GetInstance()._PlaySe(key, channel);
  }

    //PlaySeメソッドの返り値であるkey値がDictionaly変数の_poolSeのkeyに存在しているか確認
    ...
    //Dictionaly変数の_poolSeのkeyにPlaySeメソッドの返り値であるkey値が定義されていない場合
    // リソースの取得
    ...
    if (0 <= channel && channel < SE_CHANNEL)
    {
      ...
      //clip変数には、(AudioSource型)の_data変数にClip(AudioClipを持った変数)を代入
      source.clip = _data.Clip;
      source.Play();
    　source.volume = 0.5f;
    }
